...
evalPrimitive :: MonadIO m => LIVSConfigNames -> H.Heap -> Sub.SubFunctions -> T.TypeEnv -> Expr -> m Expr
evalPrimitive con h sub tenv e = do
    ...
    m <- runSMT2WithGrammar con h sub tenv $ "(set-logic ALL_SUPPORTED)\n" ++ set_x ++ "\n(check-sat)\n(get-value (x))"
